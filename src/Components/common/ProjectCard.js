...
const ProjectCard = ({project}) => {
  return (
    <a href={project.projectLink} target={`${project.isLive ? "_blank" : ""}`} rel="noreferrer" className={`${project.isLive ? "cursor-pointer" : "cursor-not-allowed"}`}>
        ...
        <div className={`absolute ${project.isLive ? "bg-green-500" : "bg-orange-500"} top-0 right-0 py-1.5 px-2.5 rounded-bl-lg bg-opacity-90 font-semibold text-sm z-30`}>
            {project.isLive ? "Live" : "Constructing"}
        </div>
